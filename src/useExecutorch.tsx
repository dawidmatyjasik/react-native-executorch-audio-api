...
export const useExecutorch = () => {
  ...
  const onRecord = () => {
    ...
    recorderRef.current.onAudioReady(async (audioEvent) => {
      try {
        audioBuffersRef.current = audioBuffersRef.current.slice(-5);
        audioBuffersRef.current.push(audioEvent.buffer);

        // Ensure we have exactly 6 buffers before processing
        if (audioBuffersRef.current.length === 6) {
